		...
		mercado_price_range: function () {
			if ($("#slider-range").length > 0) {
				...
				$("#amount").val("Rp." + $("#slider-range").slider("values", 0) +
					"K - Rp." + $("#slider-range").slider("values", 1) + "K");
			}
		},
		/* ---------------------------------------------
		...
		mercado_price_quantity: function () {
			if ($(".quantity-input").length > 0) {
				$(".quantity-input").on('click', '.btn', function (event) {
					event.preventDefault();
					var _this = $(this),
						_input = _this.siblings('.inp-qty'),
						_current_value = _this.siblings('.inp-qty').val(),
						_max_value = _this.siblings('.inp-qty').attr('data-max');
